...
/**
 cell的图
 */
@property (nonatomic, strong) NSMutableArray <UIImageView *> * iconImageView;
/**
 cell文字
 */
@property (nonatomic, strong) NSMutableArray <UILabel *> *textLabel;
...
- (instancetype)initCustomArrowheadStatusMenuWithNormalTitle:(NSArray<NSString *> *)normalTitleArray selectedTitle:(NSArray<NSString *> *)selectedTitleArray normalIcon:(NSArray<NSString *> *)normalIconArray selectedIcon:(NSArray<NSString *> *)selectedIconArray menuNormalTitleColor:(UIColor *)normalTitleColor menuSelectedTitleColor:(UIColor *)selectedTitleColor menuUnitSize:(CGSize)size menuFont:(UIFont *)font menuBackColor:(UIColor *)menuBackColor menuSegmentingLineColor:(UIColor *)separatorColor distanceFromTriggerSwitch:(CGFloat)interval menuArrowStyle:(MenuArrowStyle)arrowStyle menuPlacements:(MenuPlacements)placements showAnimationEffects:(MenuShowAnimationStyle)animation {
    ...
    NSMutableArray <NSNumber*>*defaultSingleArray = [NSMutableArray array];
    for (NSInteger i = 0; i < normalTitleArray.count; i ++) {
        ...
        [defaultSingleArray addObject:@(NO)];
    }
    NSLog(@"初始化defaultSingleArray:%ld",defaultSingleArray.count);
    return [self initCustomArrowheadStatusMenuWithNormalTitle:normalTitleArray selectedTitle:selectedTitleArray normalIcon:normalIconArray selectedIcon:selectedIconArray menuNormalTitleColorArray:nTitleColorArray menuSelectedTitleColorArray:sTitleColorArray menuUnitSize:size menuFont:font menuBackColor:menuBackColor menuSegmentingLineColor:separatorColor distanceFromTriggerSwitch:interval menuArrowStyle:arrowStyle menuPlacements:placements showAnimationEffects:animation defaultSingleArray:defaultSingleArray];
}

- (instancetype)initCustomArrowheadStatusMenuWithNormalTitle:(NSArray<NSString *> *)normalTitleArray selectedTitle:(NSArray<NSString *> *)selectedTitleArray normalIcon:(NSArray<NSString *> *)normalIconArray selectedIcon:(NSArray<NSString *> *)selectedIconArray menuNormalTitleColorArray:(NSArray<UIColor *> *)normalTitleColorArray menuSelectedTitleColorArray:(NSArray<UIColor *> *)selectedTitleColorArray menuUnitSize:(CGSize)size menuFont:(UIFont *)font menuBackColor:(UIColor *)menuBackColor menuSegmentingLineColor:(UIColor *)separatorColor distanceFromTriggerSwitch:(CGFloat)interval menuArrowStyle:(MenuArrowStyle)arrowStyle menuPlacements:(MenuPlacements)placements showAnimationEffects:(MenuShowAnimationStyle)animation defaultSingleArray:(NSMutableArray <NSNumber*>*)defaultSingleArray{
    ...
    if (self) {
        ...
        self.textLabel = [NSMutableArray array];
        
        for (int i =0; i<self.normalTitleArray.count; i++) {
            UILabel *textLabel = [[UILabel alloc] init];
            textLabel.tag =i;
            [self.textLabel addObject:textLabel];
            
        }
        self.selectedTitleArray = selectedTitleArray;// 选中状态下标题
        self.defaultSingleArray = defaultSingleArray;//默认状态
        NSLog(@"2初始化defaultSingleArray:%ld",self.defaultSingleArray.count);
        self.normalIconArray = normalIconArray;// 常态下图标
        self.iconImageView = [NSMutableArray array];
        for (int i =0; i<self.normalIconArray.count; i++) {
            UIImageView *view = [[UIImageView alloc] init];
            view.tag =i;
            [self.iconImageView addObject:view];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    //不适用重用
    UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:[NSString stringWithFormat:@"%lu-+%lu", indexPath.row, indexPath.section]];
    // 移除最后一个cell的分割线
    ...
    // 设置标题格式（默认为未选状态）
    UILabel *textLabel = self.textLabel[indexPath.row];
    // 设置菜单标题（默认为未选状态）
    textLabel.text = self.normalTitleArray[indexPath.row];
    textLabel.font = self.font;
    textLabel.textAlignment = NSTextAlignmentCenter;
    // 设置标题格式
    textLabel.frame = CGRectMake(0, 0, self.menuWidth, self.cellHeight);
    [cell.contentView addSubview:textLabel];
    
    textLabel.textColor = self.normalTitleColor[indexPath.row];
    // 设置菜单图标（默认为未选状态）
    if (self.normalIconArray && indexPath.row<self.normalIconArray.count) {
        if(!self.iconSize.width || !self.iconSize.height){
            self.iconSize = CGSizeMake(self.cellHeight-10, self.cellHeight-10);
        }
        //读取未选中图标 默认读取资源库 读取不到就读取系统
        UIImage *normalImage = [UIImage imageNamed:self.normalIconArray[indexPath.row]];
        if(!normalImage){
            normalImage = [UIImage systemImageNamed:self.normalIconArray[indexPath.row]];
        }
        //读取选中图标 默认读取资源库 读取不到就读取系统
        UIImage *selectedImage = [UIImage imageNamed:self.selectedIconArray[indexPath.row]];
        if(!selectedImage){
            selectedImage = [UIImage systemImageNamed:self.selectedIconArray[indexPath.row]];
        }
        //读取图片视图数组
        UIImageView *imageview = self.iconImageView[indexPath.row];
        imageview.tag =99;
        //设置尺寸
        CGFloat y = (self.cellHeight - self.iconSize.height)/2;
        imageview.frame = CGRectMake(y,
                                     y,
                                     self.iconSize.width,
        ...
        // 设置标题格式
        textLabel.frame = CGRectMake(CGRectGetMaxX(imageview.frame) + y, 0, self.menuWidth - self.iconSize.width - y, self.cellHeight);
        //居左
        textLabel.textAlignment = NSTextAlignmentLeft;
        //多选模式下初始化默认值为未选中
        if(self.allowsMultipleSelection){
            //如果传进来少项 自动不补全 NO
            if(indexPath.row>=self.defaultSingleArray.count){
                self.defaultSingleArray[indexPath.row] = @(NO);
            }
            
            //读取外面配置的状态修改
            BOOL isSelect = [self.defaultSingleArray[indexPath.row] boolValue];
            if(isSelect){
                //重设图标为YES
                imageview.image = selectedImage;
                //添加到选中数组
                self.defaultSingleArray[indexPath.row] = @(YES);
                //重设所在行图片视图的图像对象
                self.iconImageView[indexPath.row].image = selectedImage;
                
                //设置选择中状态
                NSIndexPath *indexPathToSelect = [NSIndexPath indexPathForRow:indexPath.row inSection:indexPath.section];
                [self.menuTableView selectRowAtIndexPath:indexPathToSelect animated:YES scrollPosition:UITableViewScrollPositionNone];
                
            }
        }else{
            //单选模式下读取默认设置
            NSLog(@"单选模式下标咯:%@",self.defaultsingleIndex);
            if(self.defaultsingleIndex !=nil && indexPath.row == [self.defaultsingleIndex intValue]){
                NSLog(@"单选模式下标:%d",[self.defaultsingleIndex intValue]);
                imageview.image = selectedImage;
                //设置选择中状态
                NSIndexPath *indexPathToSelect = [NSIndexPath indexPathForRow:indexPath.row inSection:indexPath.section];
                [self.menuTableView selectRowAtIndexPath:indexPathToSelect animated:YES scrollPosition:UITableViewScrollPositionNone];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //单选模式 必须一个的时候 重复点击已选择 会跳出不处理
    if(self.defaultMustSingle && !self.allowsMultipleSelection &&  self.singleRecordCell == cell){
        [self removeMenuView];
        return;
    }
    ...
    self.textLabel[indexPath.row].text = self.selectedTitleArray[indexPath.row];
    self.textLabel[indexPath.row].textColor = self.selectedTitleColor[indexPath.row];
    if (self.selectedIconArray) {
        UIImage *image = [UIImage imageNamed:self.selectedIconArray[indexPath.row]];
        
        if(!image){
            image = [UIImage systemImageNamed:self.selectedIconArray[indexPath.row]];
        }
        self.iconImageView[indexPath.row].image = image;
    }
...
- (void)tableView:(UITableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    self.textLabel[indexPath.row].text = self.normalTitleArray[indexPath.row];
    self.textLabel[indexPath.row].textColor = self.normalTitleColor[indexPath.row];
    if (self.normalIconArray) {
        UIImage *image = [UIImage imageNamed:self.normalIconArray[indexPath.row]];
        
        if(!image){
            image = [UIImage systemImageNamed:self.normalIconArray[indexPath.row]];
        }
        self.iconImageView[indexPath.row].image = image;
    }
    // 1、通知代理处理点击事件
    if ([self.delegate respondsToSelector:@selector(menu:didClickedMenuItemUnitWithTag:andItemUnitTitle:itemiUnitPostClickState:)]) {
        [self.delegate menu:self didClickedMenuItemUnitWithTag:indexPath.row andItemUnitTitle:self.normalTitleArray[indexPath.row] itemiUnitPostClickState:NO];
    }
    // 2、移除菜单
    [self removeMenuView];
}
...
- (void)whenClickedUncheckOwnStatus:(NSInteger)tag {
    ...
    if (cell.selected) {
        cell.selected = NO;
        self.textLabel[tag].textColor = self.normalTitleColor[tag];
        if (self.normalIconArray) {
            UIImage *image = [UIImage imageNamed:self.normalIconArray[indexPath.row]];
            ...
            self.iconImageView[tag].image = image;
...
- (NSMutableArray<NSNumber *>*)defaultSingleArray{
    if(!_defaultSingleArray){
        _defaultSingleArray = [NSMutableArray array];
    }
    return _defaultSingleArray;
}
