...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        // 设置菜单图标
        if (self.iconArray && indexPath.row<self.iconArray.count) {
            if(!self.iconSize.width || !self.iconSize.height){
                self.iconSize = CGSizeMake(self.cellHeight-10, self.cellHeight-10);
            }
            UIImage *image = [UIImage imageNamed:self.iconArray[indexPath.row]];
            if(!image){
                image = [UIImage systemImageNamed:self.iconArray[indexPath.row]];
            }
