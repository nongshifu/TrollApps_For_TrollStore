...
- (Demo9Model *)getAssetModels:(NSArray<NSString *> *)appFileModels {
    ...
    for (NSString *urlString in appFileModels) {
        if (urlString == nil || [urlString isKindOfClass:[NSNull class]]) continue;
        if ([urlString containsString:@"<null>"]) continue;
        if ([urlString containsString:MAIN_File_KEY] || [urlString containsString:ICON_KEY]) continue;
        
        NSString *encodedFile = [urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
        NSURL *fileURL = [NSURL URLWithString:encodedFile];
        if (!fileURL || ![self isMediaFileWithURL:fileURL]) continue;

        // 只处理缩略图
        if ([self isImageFileWithURL:fileURL] && [urlString containsString:@"_thumbnail_"]) {
            NSArray *comps = [urlString componentsSeparatedByString:@"_thumbnail_"];
            CGFloat duration = 0;
            if (comps.count >= 2) {
                NSString *durationStr = [comps.lastObject stringByDeletingPathExtension];
                duration = durationStr.floatValue;
            }

            // 生成正确的 videoKey
            NSString *prefixStr = comps.firstObject;
            NSURL *prefixURL = [NSURL URLWithString:prefixStr];
            NSString *videoKey = [prefixURL URLByDeletingPathExtension].lastPathComponent;
    ...
    for (NSString *urlString in appFileModels) {
        ...
        // 跳过缩略图（不显示）
        if ([urlString containsString:@"_thumbnail_"]) continue;

        // --------------------------
        // 图片
        // --------------------------
        if ([self isImageFileWithURL:fileURL]) {
            NSString *thumbUrlString = [fileURL.absoluteString stringByAppendingString:@"?size=0.2&minw=200&minh=200"];
            NSURL *thumbImageURL = [NSURL URLWithString:thumbUrlString];
            
            HXCustomAssetModel *assetModel = [HXCustomAssetModel assetWithNetworkImageURL:fileURL
                                                                           networkThumbURL:thumbImageURL
                                                                                  selected:YES];
        ...
        // --------------------------
            ...
            NSString *videoKey = [fileURL URLByDeletingPathExtension].lastPathComponent;
            NSDictionary *info = thumbDict[videoKey];
            
            NSURL *thumbURL = nil;
            CGFloat duration = 0;
            
            if (info) {
                NSString *thumbStr = info[@"thumbUrl"];
                NSLog(@"✅ 找到视频缩略图: %@", thumbStr);
                
                NSString *enc = [thumbStr stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
                thumbURL = [NSURL URLWithString:enc];
                duration = [info[@"duration"] floatValue];
            }
            
            HXCustomAssetModel *assetModel = [HXCustomAssetModel assetWithNetworkVideoURL:fileURL
                                                                                videoCoverURL:thumbURL
    ...
    NSLog(@"✅ 最终媒体数量: %lu", (unsigned long)assetModels.count);
    return Models;
}
